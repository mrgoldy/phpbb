...
class helper
{
	 ...
	 *
	 * This splits up $schema_changes into individual changes
	 * so that the changes can be chunked.
	 *
	 * @param array		$schema_changes		The schema changes from a migration
	 * @return array						The schema steps
	 */
		...
		foreach ($nested_level as $change_type => $data_depth)
		{
			if (!empty($schema_changes[$change_type]))
			{
				foreach ($schema_changes[$change_type] as $key => $value)
				{
					...
					{
						$steps[] = [
							'dbtools.perform_schema_changes', [[
									$change_type	=> [
	...
	public function reverse_update_data($steps)
	{
		...
		foreach ($steps as $step)
		{
			...
			if ($class !== 'if' && $class !== 'custom')
			{
				...
				$reversed_array[] = [$class . '.reverse', $parameters];
			}
		}
