...
class pm_compose
{
	...
	public function main($action)
	{
		...
		$load		= ($this->request->is_set_post('load')) ? true : false;
		$cancel		= ($this->request->is_set_post('cancel') && !$this->request->is_set_post('save')) ? true : false;
		$delete		= ($this->request->is_set_post('delete')) ? true : false;

		$remove_u	= ($this->request->is_set('remove_u')) ? true : false;
		$remove_g	= ($this->request->is_set('remove_g')) ? true : false;
		$add_to		= ($this->request->is_set('add_to')) ? true : false;
		$add_bcc	= ($this->request->is_set('add_bcc')) ? true : false;

		$refresh	= $this->request->is_set_post('add_file') || $this->request->is_set_post('delete_file') || $save || $load
		...
		if ($action != 'edit')
		{
			...
			if ($this->config['allow_mass_pm'] && $this->auth->acl_get('u_masspm_group'))
			{
				...
				{
					$group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '"' . ($row['group_colour'] ? ' style="color: #' . $row['group_colour'] . '"' : '') . '>' . $this->group_helper->get_name($row['group_name']) . '</option>';
				}
		...
		switch ($action)
		{
			...
			case 'quotepost':
				...
				if ($action == 'quotepost')
				{
					...
					FROM ' . $this->tables['posts'] . ' p, ' . $this->tables['topics'] . ' t, ' . $this->tables['users'] . " u
		...
		if ($submit || $preview || $refresh)
		{
			...
			if ($submit)
			{
				...
				{
					$error[] = $this->language->lang('NO_RECIPIENT');
				}
			}
		...
		if (!count($error) && $preview)
		{
			...
			$preview_signature = $this->user->data['user_sig'];
			...
			// Signature
			if ($enable_sig && $this->config['allow_sig'] && $preview_signature)
			{
		...
		// Generate PM Icons
		$s_pm_icons = false;
		if ($this->config['enable_pm_icons'])
		{
		...
		$s_hidden_fields .= ($draft_id || $this->request->is_set('draft_loaded')) ? '<input type="hidden" name="draft_loaded" value="' . (($this->request->is_set('draft_loaded')) ? $this->request->variable('draft_loaded', 0) : $draft_id) . '" />' : '';
		...
		$template_ary = [
			...
			'S_SIG_ALLOWED'			=> ($this->config['allow_sig'] && $this->config['allow_sig_pm'] && $this->auth->acl_get('u_sig')),
